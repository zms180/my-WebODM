...
def build(input_point_cloud_files, output_path, max_concurrency=8, rerun=False):
    ...
    def dir_cleanup():
        if io.dir_exists(output_path):
            log.WARNING("Removing previous EPT directory: %s" % output_path)
            shutil.rmtree(output_path)
    
        if io.dir_exists(tmpdir):
            log.WARNING("Removing previous EPT temp directory: %s" % tmpdir)
    ...
    try:
        ...
        log.WARNING("Cannot build EPT using entwine (%s), attempting with untwine..." % str(e))
        dir_cleanup()
...
def build_copc(input_point_cloud_files, output_file, convert_rgb_8_to_16=False):
    if len(input_point_cloud_files) == 0:
        log.WARNING("Cannot build COPC, no input files")
        return
    ...
    if os.path.exists(tmpdir):
        log.WARNING("Removing previous directory %s" % tmpdir)
        shutil.rmtree(tmpdir)
    cleanup = [tmpdir]
    ...
    if convert_rgb_8_to_16:
        ...
        if os.path.exists(tmpdir16):
            log.WARNING("Removing previous directory %s" % tmpdir16)
        ...
        for f in input_point_cloud_files:
            ...
            try:
                ...
                log.WARNING("Cannot convert point cloud to 16bit RGB, COPC is not going to follow the official spec: %s" % str(e))
                ok = False
                break
        if ok:
